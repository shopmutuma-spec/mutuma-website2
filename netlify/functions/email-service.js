...
export async function sendEmail({ to, subject, text, template, replyTo }) {
    const key = process.env.RESEND_API_KEY;
    const from = process.env.RESEND_FROM_EMAIL;
    if (!key || !from) throw new Error("Email sending is not configured.");
    const body = JSON.stringify({ from: `MUTUMA <${from}>`, to: [to], subject, ...(template ? { template } : { text }), ...(replyTo ? { reply_to: replyTo } : {}) });
